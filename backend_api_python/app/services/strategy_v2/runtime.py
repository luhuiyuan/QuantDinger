...
class MultiAssetSimulationBroker:
        ...
        *,
        ...
        for order in batch_orders:
            ...
            if open_price is None or not math.isfinite(open_price) or open_price <= 0:
                ...
                blocked_reason = "" if forced_liquidation else target_reason
                if side and not blocked_reason and not forced_liquidation:
    ...
    def _feasible_delta(
        ...
        *,
        ...
        lot_size: float,
        position_key: str,
        policy: MarketExecutionPolicy | None = None,
        symbol: str | None = None,
        timestamp: Any | None = None,
    ) -> tuple[float, str]:
        policy = policy or self._execution_policy(symbol or current.symbol)
        trade_date = (
            pd.Timestamp(timestamp).date()
